...
/*
 * Xiaomi's released UFS ISE module installs its tracepoint callbacks through
 * these exported hook slots.  They are function-pointer objects (rather than
 * callable functions): ufs_mtk_dbg_tp_register() invokes them with the same
 * callback/private-data pair passed to for_each_kernel_tracepoint().  Keep
 * the slots NULL until the vendor module supplies the callbacks, matching
 * the stock provider's optional-hook behaviour.
 */
void (*vh_for_xiaomi_ufs_tracepoint_register)(
		void (*fct)(struct tracepoint *tp, void *priv), void *priv);
EXPORT_SYMBOL_GPL(vh_for_xiaomi_ufs_tracepoint_register);

void (*vh_for_xiaomi_ufs_tracepoint_unregister)(
		void (*fct)(struct tracepoint *tp, void *priv), void *priv);
EXPORT_SYMBOL_GPL(vh_for_xiaomi_ufs_tracepoint_unregister);
...
static int ufs_mtk_dbg_init_procfs(void)
{
	...
	if (prEntry)
		...
		pr_info("%s: failed to create ufs_debug\n", __func__);
...
int ufs_mtk_dbg_tp_register(void)
{
	...
	FOR_EACH_INTEREST(i) {
		...
		for_each_kernel_tracepoint(__ufs_mtk_dbg_tp_register,
					   &interests[i]);
		if (vh_for_xiaomi_ufs_tracepoint_register)
			vh_for_xiaomi_ufs_tracepoint_register(
				__ufs_mtk_dbg_tp_register, &interests[i]);
	}
...
void ufs_mtk_dbg_tp_unregister(void)
{
	...
	FOR_EACH_INTEREST(i) {
		if (interests[i].init) {
			for_each_kernel_tracepoint(__ufs_mtk_dbg_tp_unregister,
						   &interests[i]);
			if (vh_for_xiaomi_ufs_tracepoint_unregister)
				vh_for_xiaomi_ufs_tracepoint_unregister(
					__ufs_mtk_dbg_tp_unregister, &interests[i]);
		}
	}
...
/*
 * Compatibility entry points used by the Dali vendor UFS module.
 *
 * The released Dali ufs-mediatek-mod-ise.ko calls these callbacks while
 * registering its Xiaomi tracepoint callbacks.  The private argument is a
 * tracepoints_table record, matching the stock provider's 32-byte record:
 * name, callback, first tracepoint, registration count, and init flag.
 * The original provider matches tracepoints by name and always uses NULL as
 * the tracepoint private-data argument.
 */
void __ufs_mtk_dbg_tp_register(struct tracepoint *tp, void *priv)
{
	struct tracepoints_table *interest = priv;
	int ret;

	if (!tp || !interest || !interest->name || !tp->name)
		return;

	if (strcmp(interest->name, tp->name))
		return;

	ret = tracepoint_probe_register(tp, interest->func, NULL);
	if (!ret) {
		interest->count++;
		interest->init = interest->count != 0;
	}
}
EXPORT_SYMBOL_GPL(__ufs_mtk_dbg_tp_register);

void __ufs_mtk_dbg_tp_unregister(struct tracepoint *tp, void *priv)
{
	struct tracepoints_table *interest = priv;
	...
	tracepoint_probe_unregister(tp, interest->func, NULL);
	if (interest->count)
		interest->count--;
	interest->init = false;
}
EXPORT_SYMBOL_GPL(__ufs_mtk_dbg_tp_unregister);
